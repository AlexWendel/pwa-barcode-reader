...
`;class tO extends HTMLElement{#B=null;#$=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tN.content.cloneNode(!0))}async connectedCallback(){this.#B=this.shadowRoot?.getElementById("historyList"),this.#$=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#D((await ti())[1]||[]),this.#B?.addEventListener("click",this.#I),this.#$?.addEventListener("click",this.#P)}disconnectedCallback(){this.#B?.removeEventListener("click",this.#I),this.#$?.removeEventListener("click",this.#P)}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await ti();if(o||!Array.isArray(i))return this.#F("bs-history-error",e),o;if(i.find(e=>e===t))return;let r=[...i,t],[s]=await tr(r);return s?(ts.error("Error setting history",s),this.#F("bs-history-error",e),s):(this.#B?.insertBefore(this.#j(t),this.#B.firstElementChild),this.#F("bs-history-success",{type:"add",message:"Barcode added to history"}),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await ti();if(o||!Array.isArray(i))return this.#F("bs-history-error",e),o;let r=i.filter(e=>e!==t),[s]=await tr(r);if(s)return ts.error("Error setting history",s),this.#F("bs-history-error",e),s;let a=this.#B?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#F("bs-history-success",{type:"remove",message:"Barcode removed from history"}),null}async empty(){let[t]=await tr([]);return t?(ts.error("Error setting history",t),this.#F("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#B?.replaceChildren(),this.#F("bs-history-success",{type:"empty",message:"History emptied successfully"}),null)}#D(t){if(!this.#B)return;this.#B.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#j(t))),this.#B.appendChild(e)}#j(t){let e,o=document.createElement("li");o.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t;let i=document.createElement("div");i.className="actions";let r=document.createElement("custom-clipboard-copy"),s=r.shadowRoot?.querySelector("button");r.setAttribute("only-icon",""),r.setAttribute("value",t),s?.setAttribute("aria-label",`Copy to clipboard ${t}`),i.appendChild(r);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("aria-label",`Remove from history ${t}`),a.innerHTML=`
    ...
    `},{once:!0}),ty.defineCustomElement();let k=t?.shadowRoot,L=k?.querySelector("video"),C=k?.querySelector('[part="actions-container"]');async function _(){if(f){ts.info("Scanning..."),s?.removeAttribute("hidden");try{let[,t]=await te(),e=await x.detect(L),i=e?.rawValue??"";if(!i)throw Error("No barcode detected");if(ta(r,i),t?.addToHistory&&o?.add(i),tc(),!t?.continueScanning){v&&(clearTimeout(v),v=null),a?.removeAttribute("hidden"),l?.setAttribute("hidden",""),C?.setAttribute("hidden","");return}}catch{}f&&(v=setTimeout(()=>_(),1e3))}}async function z(e){l?.removeAttribute("hidden"),th(e.detail.video,l),_();let o=e.target.getTrackSettings(),i=e.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(d?.addEventListener("click",T),d?.removeAttribute("hidden"),t.hasAttribute("torch")&&tp({el:d,isTorchOn:!0})),o?.zoom&&i?.zoom){let e=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=o?.zoom||1;e?.addEventListener("click",e=>{let o=e.target.closest('[data-action="zoom-in"]'),i=e.target.closest('[data-action="zoom-out"]');o&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),t.zoom=n}),e?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await ty.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,g.appendChild(o)}),s.length>1&&(g?.addEventListener("change",R),g?.removeAttribute("hidden"))}async function S(t){t.preventDefault();let e={},o=new FormData(b),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,to(e),"formats-settings"===t.target.name&&(x=await tm.create(r))}function T(e){t.torch=!t.torch,tp({el:e.currentTarget,isTorchOn:t.hasAttribute("torch")})}function R(e){if("function"!=typeof t.restartVideoStream)return;let o=e.target.value||void 0;t.restartVideoStream(o)}e.supportedFormats=w,a.addEventListener("click",function(){a?.setAttribute("hidden",""),l?.removeAttribute("hidden"),C?.removeAttribute("hidden"),_()}),n.addEventListener("resize-observer:resize",function(){th(t.shadowRoot.querySelector("video"),l)}),m.addEventListener("click",function(){p.open=!0}),b.addEventListener("change",function(t,e=0,o=!1){let i=null;if("function"!=typeof t)throw TypeError("Expected a function for first argument");return(...r)=>{clearTimeout(i),o&&!i&&t(...r),i=setTimeout(()=>{i=null,o||t(...r)},e)}}(S,500)),h.addEventListener("click",function(){u.open=!0}),document.addEventListener("visibilitychange",function(){if("hidden"===document.visibilityState)f=!1,null!=t&&"function"==typeof t.stopVideoStream&&t.stopVideoStream();else{f=!0;let t=document.querySelector("video-capture");if(!t)return;if(!t.loading&&a.hasAttribute("hidden")&&_(),"function"==typeof t.startVideoStream){let e=g?.value||void 0;t.startVideoStream(e)}}}),document.addEventListener("keydown",function(t){"Escape"===t.key&&function(){let t=!a.hidden,e=p.hasAttribute("open"),o=u.hasAttribute("open");t&&!(e||o)&&a.click()}()}),document.addEventListener("bs-history-success",function(t){let{type:e,message:o}=t.detail;"add"===e&&tb(o,{variant:"success"})}),document.addEventListener("bs-history-error",function(t){let{type:e,message:o}=t.detail;("remove"===e||"empty"===e)&&u?.hide(),tb(o,{variant:"danger"})})}();
//# sourceMappingURL=barcode-scanner-main.3d6cffce.js.map
