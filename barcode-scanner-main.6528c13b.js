...
var _relayJs = require("./services/relay.js");
var _relayJsDefault = parcelHelpers.interopDefault(_relayJs);
...
var _alertElementDefinedJs = require("@georapbox/alert-element/dist/alert-element-defined.js");
...
var _toastifyJs = require("./helpers/toastify.js");
...
(async function() {
    ...
    const sendManualCodeBtn = document.getElementById('send-manual-code');
    const SCAN_RATE_LIMIT = 1000;
    let scanTimeoutId = null;
    let shouldScan = true;
    let relayService = null;
    let barcodeReader = null;
    let videoCaptureVideoEl = null;
    let videoCaptureActionsEl = null;
    async function initializeCamera() {
        ...
        barcodeReader = await (0, _barcodeReaderJs.BarcodeReader).create(intitialFormats);
        ...
        videoCaptureVideoEl = videoCaptureShadowRoot?.querySelector('video');
        videoCaptureActionsEl = videoCaptureShadowRoot?.querySelector('[part="actions-container"]');
        bsSettingsEl.supportedFormats = supportedBarcodeFormats;
        sendManualCodeBtn.addEventListener('click', handleSendManualCodeClick);
    ...
    async function scan() {
        ...
        (0, _logJs.log).info('Scanning...');
        scanInstructionsEl?.removeAttribute('hidden');
        try {
            const [, settings] = await (0, _storageJs.getSettings)();
            const barcode = await barcodeReader.detect(videoCaptureVideoEl);
            const barcodeValue = barcode?.rawValue ?? '';
            if (!barcodeValue) throw new Error('No barcode detected');
            (0, _resultJs.createResult)(cameraResultsEl, barcodeValue);
            if (relayService) relayService.sendData(barcodeValue);
            if (settings?.addToHistory) bsHistoryEl?.add(barcodeValue);
        ...
        if (shouldScan) scanTimeoutId = setTimeout(()=>scan(), SCAN_RATE_LIMIT);
    }
    /**
   ...
   */ function handleVideoCaptureError(evt) {
        ...
        return;
        const errorMessage = error.name === 'NotAllowedError' ? `<strong>Error accessing camera</strong><br>Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.` : error.message;
   ...
   */ async function handleSettingsFormChange(evt) {
    ...
    function handleSendManualCodeClick() {
        console.log("Clicou");
        const sendTextEl = document.getElementById('send-text');
        const sendTextSpinnerEl = document.getElementById('sending-spinner');
        const barcodeValue = manualCodeInput.value;
        if (!barcodeValue) return;
        sendTextEl.style.display = 'none';
        sendTextSpinnerEl.style.display = 'block';
        if (relayService) try {
            console.log("Sending code: " + barcodeValue);
            relayService.sendData(barcodeValue);
            (0, _triggerScanEffectsJs.triggerScanEffects)();
        } catch (error) {
            console.error('Failed to send data:', error);
        }
        sendTextEl.style.display = 'block';
        sendTextSpinnerEl.style.display = 'none';
    }
    /**
   ...
   */ function handleDocumentVisibilityChange() {
        if (document.visibilityState === 'hidden') {
            ...
            if (!videoCaptureEl.loading && scanBtn.hasAttribute('hidden')) scan();
    ...
    function handleDocumentEscapeKey() {
        ...
        if (!scanBtnVisible || anyDialogOpen) return;
        scanBtn.click();
    }
    /**
   ...
   */ function handleHistorySuccess(evt) {
        ...
        if (type === 'add') (0, _toastifyJs.toastify)(message, {
            variant: 'success'
        });
    }
    /**
   ...
   */ function handleHistoryError(evt) {
        const { type, message } = evt.detail;
        if (type === 'remove' || type === 'empty') historyDialog?.hide();
        (0, _toastifyJs.toastify)(message, {
            variant: 'danger'
        });
    }
    async function initializeRelay() {
        ...
        if (!wsUrl || !roomId) {
            connectionStatusText.textContent = 'URL incorreta! Tente novamente.';
            loadingSpinner.hidden = true;
            connectionModal.hidden = false;
            return;
        }
        relayService = new (0, _relayJsDefault.default)(wsUrl, roomId);
        relayService.onStatusChange = (status, message)=>{
            switch(status){
                case 'connecting':
                    connectionStatusText.textContent = 'Conectando ao servidor...';
                    connectionModal.hidden = false;
                    break;
                case 'authenticating':
                    connectionStatusText.textContent = 'Autenticando...';
                    connectionModal.hidden = false;
                    break;
                case 'joining':
                    connectionStatusText.textContent = 'Entrando na sala...';
                    connectionModal.hidden = false;
                    break;
                case 'connected':
                    ...
                    break;
                case 'reconnecting':
                    connectionStatusText.textContent = "Conex\xe3o perdida. Reconectando...";
                    connectionModal.hidden = false;
                    break;
                case 'disconnected':
                    connectionStatusText.textContent = 'Desconectado do servidor.';
                    connectionModal.hidden = false;
                    if (typeof videoCaptureEl.stopVideoStream === 'function') videoCaptureEl.stopVideoStream();
                    break;
                case 'error':
                    connectionStatusText.textContent = message || 'Erro ao conectar ao servidor.';
                    connectionModal.hidden = false;
                    break;
            }
        };
...
},{"./services/relay.js":"8EFNI","@georapbox/a-tab-group/dist/a-tab-group.js":"kWdyV","@georapbox/web-share-element/dist/web-share-defined.js":"gkQbv","@georapbox/files-dropzone-element/dist/files-dropzone-defined.js":"aqtTK","@georapbox/resize-observer-element/dist/resize-observer-defined.js":"zv9b2","@georapbox/modal-element/dist/modal-element-defined.js":"gIK7B","@georapbox/alert-element/dist/alert-element-defined.js":"gCVhQ","./services/storage.js":"le6QT","./utils/debounce.js":"ePSA8","./utils/log.js":"d7Ofh","./utils/isDialogElementSupported.js":"1qbFt","./helpers/result.js":"eFQt0","./helpers/triggerScanEffects.js":"hcQv1","./helpers/resizeScanFrame.js":"6tibo","./helpers/BarcodeReader.js":"e32Q5","./helpers/toggleTorchButtonStatus.js":"9fowX","./components/video-capture.js":"cUPuR","./helpers/toastify.js":"cj46B","./components/clipboard-copy.js":"5zJOs","./components/bs-result.js":"6WYvJ","./components/bs-settings.js":"1xVa1","./components/bs-history.js":"ieSV7","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"kWdyV":[function(require,module,exports,__globalThis) {
/*!
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"gkQbv":[function(require,module,exports,__globalThis) {
/*!
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"aqtTK":[function(require,module,exports,__globalThis) {
/*!
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"zv9b2":[function(require,module,exports,__globalThis) {
/*!
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"gIK7B":[function(require,module,exports,__globalThis) {
/*!
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "ModalElement", ()=>n);
...
`;
var n = class s extends HTMLElement {
    #e = null;
    #a = null;
    #s = null;
    #o = void 0;
    constructor(){
        super(), this.shadowRoot || this.attachShadow({
            mode: "open"
        }).appendChild(a.content.cloneNode(!0)), this.shadowRoot && (this.#e = this.shadowRoot.querySelector("dialog"), this.#a = this.shadowRoot.querySelector('slot[name="footer"]'), this.#s = this.shadowRoot.querySelector('slot[name="close"]'));
    }
    static get observedAttributes() {
        return [
            "open",
    ...
    connectedCallback() {
        this.#t("open"), this.#t("staticBackdrop"), this.#t("noHeader"), this.#t("noAnimations"), this.#t("noCloseButton"), this.#t("fullscreen"), this.#t("preserveOverflow"), this.#t("placement"), this.#t("closeLabel"), this.#e?.addEventListener("click", this.#h), this.#e?.addEventListener("close", this.#l), this.#e?.addEventListener("cancel", this.#d), this.#e?.querySelector('form[method="dialog"]')?.addEventListener("submit", this.#c), this.#a?.addEventListener("slotchange", this.#g), this.#s?.addEventListener("slotchange", this.#u), this.addEventListener("command", this.#m);
    }
    disconnectedCallback() {
        this.#o && clearTimeout(this.#o), this.#e?.addEventListener("click", this.#h), this.#e?.removeEventListener("close", this.#l), this.#e?.removeEventListener("cancel", this.#d), this.#e?.querySelector('form[method="dialog"]')?.removeEventListener("submit", this.#c), this.#a?.removeEventListener("slotchange", this.#g), this.#s?.removeEventListener("slotchange", this.#u), this.removeEventListener("command", this.#m);
    }
    get open() {
    ...
    #r() {
        ...
        if (e === null) return;
        (this.#s?.assignedElements() || [])?.some((o)=>o.textContent?.replace(/\s/g, "") !== "") ? e.removeAttribute("aria-label") : e.setAttribute("aria-label", this.closeLabel);
    }
    #i() {
    ...
    #d = (e)=>{
        let t = this.#n("escape-key");
        this.dispatchEvent(t), t.defaultPrevented && (e.preventDefault(), !this.noAnimations && this.#i());
    };
    #c = (e)=>{
        let t = this.#n("close-button");
        this.dispatchEvent(t), t.defaultPrevented && (e.preventDefault(), !this.noAnimations && this.#i());
    };
    #h = (e)=>{
        let t = e.target, i = e.currentTarget;
        if (t === i) {
            let o = this.#n("backdrop-click");
            this.dispatchEvent(o), o.defaultPrevented || this.staticBackdrop ? !this.noAnimations && this.#i() : this.hide();
        }
        if (t instanceof HTMLElement && t.closest("[data-me-close]") !== null) {
            let o = this.#n("external-invoker");
            this.dispatchEvent(o), o.defaultPrevented ? !this.noAnimations && this.#i() : this.hide();
        }
    };
    #m = (e)=>{
        if (e.command === "--me-open" && !this.open && this.show(), e.command === "--me-close" && this.open) {
            let t = this.#n("external-invoker");
            this.dispatchEvent(t), t.defaultPrevented ? !this.noAnimations && this.#i() : this.hide();
        }
    };
    #g = ()=>{
        if (this.#e === null) return;
        ...
        if (e === null) return;
        let t = this.#a?.assignedNodes(), i = t ? t.length > 0 : !1;
        e.hidden = !i;
    };
    #u = ()=>{
        this.#r();
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"gCVhQ":[function(require,module,exports,__globalThis) {
/*!
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "AlertElement", ()=>d);
function f() {
...
var u = "alert-element", p = "alert-show", m = "alert-after-show", v = "alert-hide", l = "alert-after-hide", b = "--alert-show", E = "--alert-hide", g = "user", _ = "timeout", h = "api";
var r = f(), w = `
  :host {
    display: contents;
    box-sizing: border-box;
...
`;
var d = class a extends HTMLElement {
    #t = null;
    #o = null;
    #a = null;
    #n = void 0;
    ...
    attributeChangedCallback(t, e, o) {
        if (!(!this.isConnected || e === o)) switch(t){
            case "open":
                this.#_();
                break;
            case "duration":
    ...
    get duration() {
        ...
        let e = Number(t);
        return Number.isNaN(e) ? 1 / 0 : e;
    }
    set duration(t) {
        this.setAttribute("duration", t != null ? t.toString() : t);
    }
    get variant() {
    ...
    set closeLabel(t) {
        this.setAttribute("close-label", t != null ? t.toString() : t);
    }
    get customAnimations() {
        return this.#u;
    }
    set customAnimations(t) {
        this.#u = t;
    }
    connectedCallback() {
        this.#r("closable"), this.#r("open"), this.#r("duration"), this.#r("variant"), this.#r("closeLabel"), this.#r("customAnimations"), this.#t = this.shadowRoot?.querySelector(".alert") ?? null, this.#o = this.shadowRoot?.querySelector(".alert__close") ?? null, this.#a = this.shadowRoot?.querySelector('slot[name="close"]') ?? null, this.#o?.addEventListener("click", this.#m), this.#a?.addEventListener("slotchange", this.#v), this.addEventListener("mouseenter", this.#f), this.addEventListener("mouseleave", this.#p), this.addEventListener("command", this.#b), this.open ? (this.#t?.removeAttribute("hidden"), this.#c() && this.#l()) : this.#t?.setAttribute("hidden", ""), this.closeLabel && this.#d();
    }
    disconnectedCallback() {
        this.#i(), this.#o?.removeEventListener("click", this.#m), this.#a?.removeEventListener("slotchange", this.#v), this.removeEventListener("mouseenter", this.#f), this.removeEventListener("mouseleave", this.#p), this.removeEventListener("command", this.#b);
    }
    #_() {
        this.#i(), this.open ? (this.#c() && this.#l(), this.#t?.removeAttribute("hidden"), this.#h(p), this.#A(this.#t)?.finished.finally(()=>{
            this.#h(m);
        })) : (this.#h(v, {
            reason: this.#s
        }), this.#w(this.#t)?.finished.finally(()=>{
            this.#t?.setAttribute("hidden", ""), this.#h(l, {
                reason: this.#s
            }), this.#s = h;
        }));
    }
    #i() {
        this.#n !== void 0 && (clearTimeout(this.#n), this.#n = void 0);
    }
    #l() {
        this.#n = window.setTimeout(()=>{
            this.#s = _, this.open = !1;
        }, this.duration);
    }
    #c() {
        return this.open && this.duration !== 1 / 0;
    }
    #m = ()=>{
        this.closable && (this.#s = g, this.open = !1);
    };
    #f = ()=>{
        this.#i();
    };
    ...
    #d() {
        if (!this.#o) return;
        (this.#a?.assignedElements() || [])?.some((o)=>o.textContent?.replace(/\s/g, "") !== "") ? this.#o.removeAttribute("aria-label") : this.#o.setAttribute("aria-label", this.closeLabel);
    }
    #E() {
        let t = window.matchMedia("(prefers-reduced-motion: reduce)").matches, e = {
            show: {
                keyframes: [
                    {
                        opacity: 0,
        ...
        }, o = this.customAnimations || a.customAnimations || {}, s = (i)=>{
            let n = o[i]?.options ?? {}, c = e[i].options;
            return {
                ...c,
                ...n,
                duration: t || this.customAnimations === null || a.customAnimations === null ? 0 : n.duration ?? c.duration
            };
        };
        return {
            show: {
                keyframes: o.show?.keyframes ?? e.show.keyframes,
                options: s("show")
            },
            hide: {
                keyframes: o.hide?.keyframes ?? e.hide.keyframes,
                options: s("hide")
            }
        };
    }
    #A(t) {
        let { keyframes: e, options: o } = this.#E().show;
        return t?.animate(e, o);
    }
    #w(t) {
        let { keyframes: e, options: o } = this.#E().hide;
        return t?.animate(e, o);
    }
    #h(t, e = null) {
        let o = {
    ...
    show() {
        return this.open ? Promise.resolve() : (this.open = !0, this.#g(this, m));
    }
    hide() {
        return this.open ? (this.open = !1, this.#g(this, l)) : Promise.resolve();
    }
    toast(t = {}) {
        if (t = {
            forceRestart: !1,
            ...
            if (!t.forceRestart) return this.#e.promise;
            this.#e.resolve(), this.#e.cleanup();
        }
        let o = ()=>{}, s = new Promise((c)=>o = c), i = ()=>{
            this.#e?.resolve(), this.#e?.cleanup();
        };
        this.#e = {
            promise: s,
            resolve: o,
            cleanup: ()=>{
                this.removeEventListener(l, i), this.parentNode === r && r.removeChild(this), r.querySelector(u) || r.remove(), this.open = !1, this.#e = null;
            }
        }, r.parentElement || document.body.append(r), r.appendChild(this), this.#t?.setAttribute("data-toast", ""), this.open = !0;
        let n = r.shadowRoot?.querySelector(".stack");
        return n?.scrollTo({
            top: n.scrollHeight
        }), this.addEventListener(l, i, {
            once: !0
        }), s;
    }
    #r(t) {
    ...
    static defineCustomElement(t = u) {
        typeof window < "u" && !window.customElements.get(t) && window.customElements.define(t, a);
    }
};
d.defineCustomElement();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"le6QT":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"idb-keyval":"4xB8E","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"4xB8E":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"ePSA8":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"d7Ofh":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"1qbFt":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"eFQt0":[function(require,module,exports,__globalThis) {
/**
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "createResult", ()=>createResult);
async function createResult(containerEl, value) {
    if (!containerEl || !value) return;
    // const prevResultEl = containerEl.querySelector('bs-result[value="' + value + '"]');
    const prevResultEl = containerEl.querySelector(`bs-result[value="${value}"]`);
    if (prevResultEl) prevResultEl.remove();
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"hcQv1":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
...
async function triggerScanEffects(options = {}) {
    ...
    if (!settings) return;
    const now = Date.now();
    if (now - lastTriggerTime < RATE_LIMIT) return;
...
},{"../services/storage.js":"le6QT","../utils/beep.js":"ACIzj","../utils/vibrate.js":"imRIk","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"ACIzj":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"imRIk":[function(require,module,exports,__globalThis) {
/**
 * Vibrate the device.
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"6tibo":[function(require,module,exports,__globalThis) {
/**
...
function resizeScanFrame(videoEl, scanFrameEl) {
    ...
    scanFrameEl.style.cssText = `width: ${width}px; height: ${height}px`;
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"e32Q5":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
 ...
 */ class BarcodeReader {
    static async polyfill() {
        if (!('BarcodeDetector' in window)) try {
            ...
            (0, _logJs.log).info('Using BarcodeDetector polyfill.');
        } catch (error) {
            throw new Error('BarcodeDetector API is not supported by your browser.', {
                cause: error
            });
        }
        else (0, _logJs.log).info('Using the native BarcodeDetector API.');
    }
    /**
...
},{"../utils/log.js":"d7Ofh","bfcca166d6641c34":"cx3BN","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"cx3BN":[function(require,module,exports,__globalThis) {
module.exports = import("./es.0fa745cd.js").then(()=>module.bundle.root('lxAJT'));

},{"lxAJT":"lxAJT"}],"9fowX":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"cUPuR":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"../utils/clamp.js":"5VloR","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"5VloR":[function(require,module,exports,__globalThis) {
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"cj46B":[function(require,module,exports,__globalThis) {
/**
 ...
 */ var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "toastify", ()=>toastify);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"5zJOs":[function(require,module,exports,__globalThis) {
...
},{"@georapbox/clipboard-copy-element/dist/clipboard-copy.js":"c58LC"}],"c58LC":[function(require,module,exports,__globalThis) {
/*!
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"6WYvJ":[function(require,module,exports,__globalThis) {
var _isWebShareSupportedJs = require("@georapbox/web-share-element/dist/is-web-share-supported.js");
var _storageJs = require("../services/storage.js");
var _datetimeFormatterJs = require("../utils/datetime-formatter.js");
const styles = /* css */ `
  :host {
    --color-flash: #ffff99;
...
class BSResult extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (window.matchMedia('(prefers-reduced-motion: no-preference)').matches) {
            ...
            baseEl?.animate([
                {
                    backgroundColor: 'var(--color-flash)'
                },
                {
    ...
    async #handleValueChange(value) {
        ...
        let resultEl;
        if (oldResultEl) oldResultEl?.remove();
        try {
            const [, settings] = await (0, _storageJs.getSettings)();
            new URL(value);
            ...
            if (settings?.openWebPage) resultEl.click();
            else window.requestAnimationFrame(()=>resultEl.focus());
        } catch  {
        ...
        resultDatetimeEl.textContent = (0, _datetimeFormatterJs.dateTimeFormatter).format(new Date());
        ...
        if (shareEl && (0, _isWebShareSupportedJs.isWebShareSupported)()) {
...
},{"@georapbox/web-share-element/dist/is-web-share-supported.js":"2olRd","../services/storage.js":"le6QT","../utils/datetime-formatter.js":"eo4T5"}],"2olRd":[function(require,module,exports,__globalThis) {
/*!
 * @georapbox/web-share-element
 * A custom element that implements the Web Share API to share user-defined data.
 *
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"eo4T5":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"1xVa1":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"../services/storage.js":"le6QT","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"ieSV7":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class BSHistory extends HTMLElement {
   ...
   */ async add(item) {
        ...
        if (history.find((h)=>h === item)) return;
        const data = [
            ...history,
            item
        ];
        ...
        if (setHistoryError) {
            (0, _logJs.log).error('Error setting history', setHistoryError);
            this.#emitEvent('bs-history-error', errPayload);
            return setHistoryError;
        }
        this.#historyListEl?.insertBefore(this.#createHistoryItemElement(item), this.#historyListEl.firstElementChild);
   ...
   */ #renderHistoryList(data) {
        ...
        [
            ...data
        ].reverse().forEach((item)=>fragment.appendChild(this.#createHistoryItemElement(item)));
...
},{"../services/storage.js":"le6QT","../utils/log.js":"d7Ofh","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["6DHTQ","6kb64"], "6kb64", "parcelRequirea202", {})

//# sourceMappingURL=barcode-scanner-main.6528c13b.js.map
