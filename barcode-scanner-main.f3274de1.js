...
`;class tH extends HTMLElement{#B=null;#D=null;constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tO.content.cloneNode(!0))}async connectedCallback(){this.#B=this.shadowRoot?.getElementById("historyList"),this.#D=this.shadowRoot?.getElementById("emptyHistoryBtn"),this.#$((await tr())[1]||[]),this.#B?.addEventListener("click",this.#I),this.#D?.addEventListener("click",this.#P)}disconnectedCallback(){this.#B?.removeEventListener("click",this.#I),this.#D?.removeEventListener("click",this.#P)}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await tr();if(o||!Array.isArray(i))return this.#F("bs-history-error",e),o;if(i.find(e=>e===t))return;let r=[...i,t],[s]=await ts(r);return s?(ta.error("Error setting history",s),this.#F("bs-history-error",e),s):(this.#B?.insertBefore(this.#j(t),this.#B.firstElementChild),this.#F("bs-history-success",{type:"add",message:"Barcode added to history"}),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await tr();if(o||!Array.isArray(i))return this.#F("bs-history-error",e),o;let r=i.filter(e=>e!==t),[s]=await ts(r);if(s)return ta.error("Error setting history",s),this.#F("bs-history-error",e),s;let a=this.#B?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#F("bs-history-success",{type:"remove",message:"Barcode removed from history"}),null}async empty(){let[t]=await ts([]);return t?(ta.error("Error setting history",t),this.#F("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#B?.replaceChildren(),this.#F("bs-history-success",{type:"empty",message:"History emptied successfully"}),null)}#$(t){if(!this.#B)return;this.#B.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#j(t))),this.#B.appendChild(e)}#j(t){let e,o=document.createElement("li");o.setAttribute("data-value",t);try{new URL(t),(e=document.createElement("a")).href=t,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=t;let i=document.createElement("div");i.className="actions";let r=document.createElement("custom-clipboard-copy"),s=r.shadowRoot?.querySelector("button");r.setAttribute("only-icon",""),r.setAttribute("value",t),s?.setAttribute("aria-label",`Copy to clipboard ${t}`),i.appendChild(r);let a=document.createElement("button");return a.type="button",a.className="delete-action",a.setAttribute("data-action","delete"),a.setAttribute("aria-label",`Remove from history ${t}`),a.innerHTML=`
    ...
    `,i.appendChild(a),o.appendChild(e),o.appendChild(i),o}#I=async t=>{let e=t.target;if(e.closest('[data-action="delete"]')){let t=e.closest("li").dataset.value;window.confirm(`Delete history item ${t}?`)&&this.remove(t)}};#P=async()=>{window.confirm("Empty history? This action cannot be undone.")&&this.empty()};#F(t,e=null){let o=new CustomEvent(t,{bubbles:!0,composed:!0,detail:e});this.dispatchEvent(o)}static defineCustomElement(t="bs-history"){"undefined"==typeof window||window.customElements.get(t)||window.customElements.define(t,tH)}}tH.defineCustomElement(),async function(){let t=document.querySelector("video-capture"),e=document.querySelector("bs-settings"),o=document.querySelector("bs-history"),i=document.getElementById("cameraPanel"),r=i.querySelector(".results"),s=document.getElementById("scanInstructions"),a=document.getElementById("scanBtn"),n=document.querySelector("resize-observer"),d=document.getElementById("scanFrame"),c=document.getElementById("torchButton"),h=document.getElementById("globalActions"),u=document.getElementById("historyBtn"),m=document.getElementById("historyDialog"),p=document.getElementById("settingsBtn"),b=document.getElementById("settingsDialog"),g=document.getElementById("settingsForm"),v=document.getElementById("cameraSelect"),f=document.getElementById("connectionModal"),y=document.getElementById("connectionStatusText"),w=null,E=!0,A=null,x=null,k=null,L=null;async function C(){let{barcodeReaderError:o}=await tp.setup();if(o){let t=document.getElementById("barcodeReaderError");E=!1,h?.setAttribute("hidden",""),t?.setAttribute("open","");return}let i=await tp.getSupportedFormats(),[,r]=await to(),s=r?.formats||i;x=await tp.create(s),t.addEventListener("video-capture:video-play",T,{once:!0}),t.addEventListener("video-capture:error",R,{once:!0}),tw.defineCustomElement();let l=t?.shadowRoot;k=l?.querySelector("video"),L=l?.querySelector('[part="actions-container"]'),e.supportedFormats=i,a.addEventListener("click",S),n.addEventListener("resize-observer:resize",z),p.addEventListener("click",M),g.addEventListener("change",function(t,e=0,o=!1){let i=null;if("function"!=typeof t)throw TypeError("Expected a function for first argument");return(...r)=>{clearTimeout(i),o&&!i&&t(...r),i=setTimeout(()=>{i=null,o||t(...r)},e)}}(q,500)),u.addEventListener("click",N),document.addEventListener("visibilitychange",B),document.addEventListener("keydown",D),document.addEventListener("bs-history-success",$),document.addEventListener("bs-history-error",I)}async function _(){if(E){ta.info("Scanning..."),s?.removeAttribute("hidden");try{let[,t]=await to(),e=await x.detect(k),i=e?.rawValue??"";if(!i)throw Error("No barcode detected");if(tn(r,i),A&&A.sendData(i),t?.addToHistory&&o?.add(i),th(),!t?.continueScanning){w&&(clearTimeout(w),w=null),a?.removeAttribute("hidden"),d?.setAttribute("hidden",""),L?.setAttribute("hidden","");return}}catch{}E&&(w=setTimeout(()=>_(),1e3))}}function S(){a?.setAttribute("hidden",""),d?.removeAttribute("hidden"),L?.removeAttribute("hidden"),_()}function z(){tu(t.shadowRoot.querySelector("video"),d)}async function T(e){d?.removeAttribute("hidden"),tu(e.detail.video,d),_();let o=e.target.getTrackSettings(),i=e.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(c?.addEventListener("click",O),c?.removeAttribute("hidden"),t.hasAttribute("torch")&&tb({el:c,isTorchOn:!0})),o?.zoom&&i?.zoom){let e=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=o?.zoom||1;e?.addEventListener("click",e=>{let o=e.target.closest('[data-action="zoom-in"]'),i=e.target.closest('[data-action="zoom-out"]');o&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),t.zoom=n}),e?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await tw.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,v.appendChild(o)}),s.length>1&&(v?.addEventListener("change",H),v?.removeAttribute("hidden"))}function R(t){let e=t.detail.error;if("NotFoundError"===e.name)return;let o="NotAllowedError"===e.name?"<strong>Error accessing camera</strong><br>Permission to use webcam was denied or video Autoplay is disabled. Reload the page to give appropriate permissions to webcam.":e.message;i.innerHTML=`
    ...
    `}function M(){b.open=!0}async function q(t){t.preventDefault();let e={},o=new FormData(g),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,ti(e),"formats-settings"===t.target.name&&(x=await tp.create(r))}function N(){m.open=!0}function O(e){t.torch=!t.torch,tb({el:e.currentTarget,isTorchOn:t.hasAttribute("torch")})}function H(e){if("function"!=typeof t.restartVideoStream)return;let o=e.target.value||void 0;t.restartVideoStream(o)}function B(){if("hidden"===document.visibilityState)E=!1,null!=t&&"function"==typeof t.stopVideoStream&&t.stopVideoStream();else{E=!0;let t=document.querySelector("video-capture");if(!t)return;if(!t.loading&&a.hasAttribute("hidden")&&_(),"function"==typeof t.startVideoStream){let e=v?.value||void 0;t.startVideoStream(e)}}}function D(t){"Escape"===t.key&&function(){let t=!a.hidden,e=b.hasAttribute("open"),o=m.hasAttribute("open");t&&!(e||o)&&a.click()}()}function $(t){let{type:e,message:o}=t.detail;"add"===e&&tg(o,{variant:"success"})}function I(t){let{type:e,message:o}=t.detail;("remove"===e||"empty"===e)&&m?.hide(),tg(o,{variant:"danger"})}async function P(){let e=new URLSearchParams(window.location.search),o=e.get("ws_url"),i=e.get("room_id");if(!o||!i){y.textContent="ws_url and room_id parameters are required.",f.hidden=!1;return}(A=new(0,l.default)(o,i)).onStatusChange=(e,o)=>{switch(e){case"connecting":y.textContent="Connecting to relay server...",f.hidden=!1;break;case"authenticating":y.textContent="Authenticating...",f.hidden=!1;break;case"joining":y.textContent="Joining room...",f.hidden=!1;break;case"connected":f.hidden=!0,C();break;case"reconnecting":y.textContent="Connection lost. Reconnecting...",f.hidden=!1;break;case"disconnected":y.textContent="Disconnected from relay server.",f.hidden=!1,"function"==typeof t.stopVideoStream&&t.stopVideoStream();break;case"error":y.textContent=o||"Error connecting to relay server.",f.hidden=!1}},A.connect()}"function"==typeof HTMLDialogElement&&(h?.removeAttribute("hidden"),m?.removeAttribute("hidden"),b?.removeAttribute("hidden")),P()}();
//# sourceMappingURL=barcode-scanner-main.f3274de1.js.map
